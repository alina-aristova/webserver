...
ConnectionClass::ConnectionClass(int socket, std::map<std::string, HostClass> hosts) : _socket(socket),
_closeConnection(false), _responseStatus("200"), _writingBuff(""), _stateOfConnection(IS_PROCESSING_FIRST_LINE),
_writer(new WritingTransmitterClass(_socket, _responseStatus, _stateOfConnection, _writingBuff, _closeConnection)),
_reader(new ReadingTransmitterClass(_socket, _responseStatus, _stateOfConnection, _writingBuff, _closeConnection)) {}
